...
class PhraseBuilder:
    ...
    def build(self) -> PhraseContainer:
        return PhraseContainer(
            metadata=self.metadata, patterns=self.patterns, whitelistedTerms=self.whitelistedTerms
        )
...
class Dataset:
    ...
    def get_payload_with_phrase_metadata(self, payload: MatchPayload) -> any:
        ...
        return {**payload.__dict__, "phraseMetadata": self.containers[offset].metadata}

    def build(self) -> dict:
        return {
            "blacklisted_terms": assign_incrementing_ids(
                [pattern for container in self.containers for pattern in container.patterns]
            ),
            "whitelisted_terms": assign_incrementing_ids(
