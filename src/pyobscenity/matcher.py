...
class Matcher(abc.ABC):
    ...
    @abc.abstractmethod
    def get_all_matches(self, input: str, sorted: bool | None = False) -> list[MatchPayload]:
        """
...
class RegexMatcher(Matcher):
    ...
    def get_all_matches(self, input: str, sorted: bool | None = False) -> list[MatchPayload]:
        ...
        transformedToOrigIndex, transformed = self.apply_transformers(
            input, self.blacklist_transformers
        )
        ...
        for term in self.compiled_blacklisted_terms:
            for match in term["regex"].finditer(transformed):
                ...
                if (
                    orig_end < len(input)
                    and is_high_surrogate(input[orig_end - 1])
                    and is_low_surrogate(input[orig_end])
                ):
                    orig_end += 1
    ...
    def get_whitelisted_intervals(self, input: str) -> list[tuple[int, int]]:
        intervals = []
        transformedToOrigIndex, transformed = self.apply_transformers(
            input, self.whitelist_transformers
        )
    ...
    def apply_transformers(
        self, input: str, transformers: list[Transformer]
    ) -> tuple[list[int], str]:
        transformed = input
    ...
    def compile_terms(self):
        ...
        for term in self.blacklisted_terms:
            self.compiled_blacklisted_terms.append(
                {
                    "id": term["id"],
                    "regex": regex.compile(term["pattern"].as_regex())
                    if hasattr(term["pattern"], "as_regex")
                    else regex.compile(term["pattern"]),
                }
            )
