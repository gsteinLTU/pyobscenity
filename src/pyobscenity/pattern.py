...
Node = LiteralNode | OptionalNode | WildcardNode | BoundaryAssertionNode
...
class ParsedPattern:
    ...
    def as_regex(self) -> str:
        ...
        if (
            self.require_word_boundary_at_start
            and len(self.nodes) > 0
            and not isinstance(self.nodes[0], BoundaryAssertionNode)
        ):
        ...
        if (
            self.require_word_boundary_at_end
            and len(self.nodes) > 0
            and not isinstance(self.nodes[-1], BoundaryAssertionNode)
        ):
            regex_parts.append(r"\b")
...
class PatternParser:
    ...
    def parse_pattern(self, pattern: str) -> ParsedPattern:
        ...
        while True:
            ...
            if isinstance(node, BoundaryAssertionNode):
                if not self.done:
                    raise ValueError(
                        f"Unexpected word boundary assertion at line {self.line}, column {self.column}"
                    )
    ...
    def _parse_optional(self) -> OptionalNode:
        ...
        if self._accept("["):
            raise ValueError(
                f"Nested optionals are not allowed at line {self.line}, column {self.column}"
            )
        ...
        if isinstance(child_node, BoundaryAssertionNode):
            raise ValueError(
                f"Boundary assertions are not allowed inside optionals at line {self.line}, column {self.column}"
            )
        ...
        while not self.done:
            ...
            if next_char == "\\":
                if self.done:
                    self._backup()
                    raise ValueError(
                        f"Trailing backslash at line {self.line}, column {self.column}"
                    )
                escaped_char = self._next()
                if escaped_char and escaped_char not in supports_escaping:
                    self._backup()
                    raise ValueError(
                        f"Invalid escape sequence '\\{escaped_char}' at line {self.line}, column {self.column}"
                    )
