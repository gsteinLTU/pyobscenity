...
def test_keep_start_censor():
    ...
    output_text = keep_start_censor(
        input_text, [MatchPayload(0, len(input_text), len(input_text), 1)]
    )
    assert output_text == "Se###########", "KeepStartCensor failed."

    input_text2 = "DataSensitive"
    output_text2 = keep_start_censor(
        input_text2, [MatchPayload(4, len(input_text2), len(input_text2) - 4, 1)]
    )
...
def test_keep_end_censor():
    ...
    input_text = "SensitiveData"
    output_text = keep_end_censor(
        input_text, [MatchPayload(0, len(input_text), len(input_text), 1)]
    )
    assert output_text == "#########Data", "KeepEndCensor failed."

    input_text2 = "DataSensitive"
    output_text2 = keep_end_censor(
        input_text2, [MatchPayload(4, len(input_text2), len(input_text2) - 4, 1)]
    )
...
def test_random_char_censor():
    ...
    input_text = "SensitiveData"
    output_text = random_char_censor(
        input_text, [MatchPayload(0, len(input_text), len(input_text), 1)]
    )
    ...
    input_text2 = "DataSensitive"
    output_text2 = random_char_censor(
        input_text2, [MatchPayload(4, len(input_text2), len(input_text2) - 4, 1)]
    )
