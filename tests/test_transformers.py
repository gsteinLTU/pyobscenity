...
def test_identity_transformer():
    ...
    """
    identity_transformer = Transformer(transform_func=lambda text: text)
...
def test_collapse_duplicate_transformer():
    ...
    custom_char_transformer = CollapseDuplicateTransformer(
        3,
        {
            "o": 2,
            "l": 1,
    ...
    assert output_text4 == "heloo!!!", (
        "CollapseDuplicate transformer with custom char thresholds failed."
    )

    # Test with None threshold
    none_threshold_transformer = CollapseDuplicateTransformer(
        None,
        {
            " ": 1,
        },
    ...
    assert output_text5 == "This is a test.", (
        "CollapseDuplicate transformer with None threshold failed."
    )
